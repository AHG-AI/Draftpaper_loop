...
def _release_package_builder_module() -> Any:
    builder_path = REPO_ROOT / "scripts" / "package_release.py"
    spec = importlib.util.spec_from_file_location("draftpaper_package_release", builder_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load scripts/package_release.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def _release_package_verifier_module() -> Any:
...
def _support_bundle_verifier_module() -> Any:
    verifier_path = REPO_ROOT / "scripts" / "verify_support_bundle.py"
    spec = importlib.util.spec_from_file_location("draftpaper_verify_support_bundle", verifier_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load scripts/verify_support_bundle.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
def _commercial_launch_package_builder_module() -> Any:
    builder_path = REPO_ROOT / "scripts" / "build_commercial_launch_package.py"
    spec = importlib.util.spec_from_file_location("draftpaper_build_commercial_launch_package", builder_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load scripts/build_commercial_launch_package.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def _commercial_launch_package_verifier_module() -> Any:
    verifier_path = REPO_ROOT / "scripts" / "verify_commercial_launch_package.py"
    spec = importlib.util.spec_from_file_location("draftpaper_verify_commercial_launch_package", verifier_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load scripts/verify_commercial_launch_package.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
def _verified_release_installer_module() -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load scripts/install_verified_release.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
def build_release_package_from_console(
    *,
    output_dir: str = "",
    version_label: str = "",
    signing_key: str = "",
    ...
    _require_role(context, {"admin"})
    generated_at = _utc_timestamp()
    label = version_label.strip() or f"commercial-console-{generated_at}"
    package_root = RUNTIME_ROOT / "release_packages" / _safe_slug(label)
    output_dir_path = Path(output_dir).expanduser().resolve() if output_dir.strip() else package_root
    builder = _release_package_builder_module()
    package = builder.build_release_package(
        root=REPO_ROOT,
        output_dir=output_dir_path,
        version_label=label,
        signing_key=Path(signing_key).expanduser().resolve() if signing_key.strip() else None,
    ...
    verifier = _release_package_verifier_module()
    verification = verifier.verify_release_package(
        Path(str(package.get("zip_path") or "")).expanduser().resolve(),
        manifest_path=Path(str(package.get("manifest_path") or "")).expanduser().resolve() if package.get("manifest_path") else None,
        sha256_path=Path(str(package.get("sha256_path") or "")).expanduser().resolve() if package.get("sha256_path") else None,
        signature_path=Path(str(signature.get("signature_path") or "")).expanduser().resolve() if signature.get("signature_path") else None,
        public_key_path=Path(str(signature.get("public_key_path") or "")).expanduser().resolve() if signature.get("public_key_path") else None,
        require_signature=require_signature or bool(signature),
    )
    missing_required = package.get("missing_required_files") if isinstance(package.get("missing_required_files"), list) else []
    status = "verified" if not missing_required and verification.get("status") == "verified" else "attention"
    append_audit(
    ...
    return {
        "schema_version": "draftpaper.release-package-build/v1",
        "status": status,
        "generated_at": generated_at,
        "output_dir": str(output_dir_path),
        "version_label": label,
        ...
        "verification": verification,
        "notes": [
            "This builds a source release package and immediately verifies the generated zip, external manifest, and SHA256 sidecar.",
            "When a signing key is supplied, the detached signature is verified but the private key path is not recorded in the response.",
            "Default outputs are written under the private console runtime root.",
        ],
    }


def build_commercial_launch_package_from_console(
    *,
    acceptance_report: str,
    release_zip: str,
    release_manifest: str,
    release_sha256_file: str,
    ...
    if missing:
        raise ValueError(f"missing required launch package inputs: {', '.join(missing)}")
    target_track = target_track.strip() or "paid_local_handoff"
    generated_at = _utc_timestamp()
    customer_slug = _safe_slug(customer_id or customer_name or target_track)
    launch_root = RUNTIME_ROOT / "commercial_launch_packages" / _safe_slug(f"{customer_slug}-{target_track}-{generated_at}")
    output_dir_path = Path(output_dir).expanduser().resolve() if output_dir.strip() else launch_root
    output_zip_path = Path(output_zip).expanduser().resolve() if output_zip.strip() else None
    hosted_path = Path(hosted_readiness_dossier).expanduser().resolve() if hosted_readiness_dossier.strip() else None
    builder = _commercial_launch_package_builder_module()
    launch = builder.build_commercial_launch_package(
        acceptance_report=Path(acceptance_report).expanduser().resolve(),
        output_dir=output_dir_path,
        output_zip=output_zip_path,
        target_track=target_track,
        customer_id=customer_id.strip(),
        customer_name=customer_name.strip(),
        release_zip=Path(release_zip).expanduser().resolve(),
        release_manifest=Path(release_manifest).expanduser().resolve(),
        release_sha256_file=Path(release_sha256_file).expanduser().resolve(),
        release_signature=Path(release_signature).expanduser().resolve(),
        release_public_key=Path(release_public_key).expanduser().resolve(),
        handoff_dossier=Path(handoff_dossier).expanduser().resolve(),
        support_bundle=Path(support_bundle).expanduser().resolve(),
        hosted_readiness_dossier=hosted_path,
    )
    verification: dict[str, Any] = {}
    zip_path_text = str(launch.get("zip_path") or "")
    zip_path = Path(zip_path_text).expanduser().resolve() if zip_path_text else None
    if zip_path is not None and zip_path.exists():
        verifier = _commercial_launch_package_verifier_module()
        verification = verifier.verify_commercial_launch_package(zip_path)
    status = "verified" if launch.get("status") == "ready" and verification.get("status") == "verified" else "attention"
    append_audit(
    ...
    return {
        "schema_version": "draftpaper.commercial-launch-package-build/v1",
        "status": status,
        "generated_at": generated_at,
        "target_track": target_track,
        "customer_id": customer_id.strip(),
        ...
        "output_dir": str(output_dir_path),
        "zip_path": str(zip_path) if zip_path is not None else "",
        "package_path": launch.get("package_path"),
        "summary_path": launch.get("summary_path"),
        "zip_sha256": launch.get("zip_sha256"),
        "package_sha256": launch.get("package_sha256"),
        "launch_status": launch.get("status"),
        ...
        "verification": verification,
        "notes": [
            "This builds a commercial launch evidence package and independently reverifies the generated zip.",
            "The package includes distributable release, dossier, and support artifacts while excluding raw acceptance payloads and private runtime secrets.",
...
def install_verified_release_from_console(
    *,
    release_zip: str,
    install_root: str = "",
    release_manifest: str = "",
    ...
    context = context or _local_admin_context()
    _require_role(context, {"admin"})
    if not release_zip.strip():
        raise ValueError("release_zip is required")
    release_path = Path(release_zip).expanduser().resolve()
    generated_at = _utc_timestamp()
    release_slug = _safe_slug(release_path.stem)
    default_root = RUNTIME_ROOT / "verified_release_installs" / _safe_slug(f"{release_slug}-{generated_at}")
    install_root_path = Path(install_root).expanduser().resolve() if install_root.strip() else default_root
    smoke_output_path = Path(smoke_output).expanduser().resolve() if smoke_output.strip() else None

    installer = _verified_release_installer_module()
    result = installer.install_verified_release(
        release_zip=release_path,
        install_root=install_root_path,
        release_manifest=Path(release_manifest).expanduser().resolve() if release_manifest.strip() else None,
        release_sha256_file=Path(release_sha256_file).expanduser().resolve() if release_sha256_file.strip() else None,
        release_signature=Path(release_signature).expanduser().resolve() if release_signature.strip() else None,
        release_public_key=Path(release_public_key).expanduser().resolve() if release_public_key.strip() else None,
        handoff_dossier=Path(handoff_dossier).expanduser().resolve() if handoff_dossier.strip() else None,
    ...
    append_audit(
        "verified_release_installed",
        actor=str(context.get("id") or "unknown"),
        status=result.get("status"),
        release_zip=str(release_path),
        install_root=str(install_root_path),
        installed_dir=result.get("installed_dir"),
        run_smoke=run_smoke,
    )
    return {
        "schema_version": "draftpaper.verified-release-install-run/v1",
        "status": result.get("status"),
        "generated_at": generated_at,
        "release_zip": str(release_path),
        "install_root": str(install_root_path),
        "installed_dir": result.get("installed_dir") or "",
        "install_manifest": result.get("install_manifest") or "",
        "installed_smoke_output": result.get("installed_smoke_output") or "",
        "summary": result.get("summary") if isinstance(result.get("summary"), dict) else {},
        "release_verification_status": (result.get("release_verification") or {}).get("status") if isinstance(result.get("release_verification"), dict) else "",
        "dossier_verification_status": (result.get("dossier_verification") or {}).get("status") if isinstance(result.get("dossier_verification"), dict) else "",
        "installed_smoke_status": (result.get("installed_smoke") or {}).get("status") if isinstance(result.get("installed_smoke"), dict) else "",
        "result": result,
        "notes": [
            "This verifies a release package before extraction and writes an owner-only install manifest.",
            "When smoke is enabled, the installed release is tested locally without external literature providers, hosted services, customer jobs, or data imports.",
            "Default installs are written under the private console runtime root.",
        ],
    }
...
def build_support_bundle_file_from_console(
    *,
    output_path: str = "",
    context: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    default_dir = RUNTIME_ROOT / "support_bundles" / _safe_slug(generated_at)
    zip_path = Path(output_path).expanduser().resolve() if output_path.strip() else default_dir / "draftpaper-support-bundle.zip"
    zip_path.parent.mkdir(parents=True, exist_ok=True)
    zip_path.parent.chmod(0o700)
    zip_path.write_bytes(archive)
    zip_path.chmod(0o600)
    verifier = _support_bundle_verifier_module()
    verification = verifier.verify_support_bundle(zip_path)
    status = "verified" if verification.get("status") == "verified" else "attention"
    append_audit(
        "support_bundle_file_built",
        actor=str(context.get("id") or "unknown"),
        status=status,
    ...
    return {
        "schema_version": "draftpaper.support-bundle-file/v1",
        "status": status,
        "generated_at": generated_at,
        "zip_path": str(zip_path),
        "archive_bytes": len(archive),
        ...
        "verification": verification,
        "notes": [
            "This writes the same redacted diagnostics bundle as /api/support-bundle to a private local path.",
            "The generated file is immediately verified with scripts/verify_support_bundle.py.",
            "Use zip_path as the support_bundle input for commercial launch packages.",
        ],
    }
...
def _commercial_capability_checks() -> list[tuple[str, bool, str]]:
    return [
        ...
        ("release_package_console", True, "Source release packages can be built and independently verified from the console."),
        ...
        ("support_bundle_file_console", True, "Support bundles can be written to a private local file and reverified from the console."),
        ...
        ("commercial_launch_package_console", True, "Commercial launch packages can be built and reverified from the console."),
...
def handoff_readiness(*, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    local_pilot_gates = [
        ...
        _security_check("release_package_console", bool(capability_map.get("release_package_console")), "error", "Release package can be built and verified from the console.", "Restore /api/release-package."),
        ...
        _security_check("support_bundle_file_console", bool(capability_map.get("support_bundle_file_console")), "error", "Support bundle can be written to a private local file from the console.", "Restore /api/support-bundle-file."),
        ...
        _security_check("commercial_launch_package_console", bool(capability_map.get("commercial_launch_package_console")), "error", "Commercial launch package can be built from the console.", "Restore /api/commercial-launch-package."),
...
def _evidence_summary(report: dict[str, Any]) -> dict[str, Any]:
    summary = report.get("summary") if isinstance(report.get("summary"), dict) else {}
    return {
        "checks": int(summary.get("checks") or summary.get("total") or 0),
...
def _commercial_evidence_item(
    *,
    ...
    configured = bool(evidence_paths) or status not in {"unconfigured", "not_ready", "unknown"}
    ...
    return {
        ...
        "blocking": bool(required and not passed),
        "next_action": "" if passed else next_action,
    }


def _commercial_evidence_markdown(pack: dict[str, Any]) -> str:
    lines = [
        "# Draftpaper Commercial Evidence Pack",
        "",
        f"Generated at: {pack.get('generated_at')}",
        f"Target track: {pack.get('target_track')}",
    ...
    for item in pack.get("evidence", []):
        marker = "OK" if item.get("passed") else "TODO"
        required = "required" if item.get("required") else "optional"
        lines.append(f"- [{marker}] {item.get('label')} - {item.get('status')} ({required})")
    ...
    if actions:
        ...
        lines.append("- No blocking evidence actions for the selected track.")
    lines.extend(
        [
            "",
            "## Boundary",
            "",
            "This pack records evidence status and private file locations. It does not create legal approval, security review, release trust, scientific claim confirmation, payment collection, SSO, or hosted production readiness by itself.",
            "",
        ]
    )
    return "\n".join(lines)
...
def build_commercial_evidence_pack_from_console(
    *,
    output_dir: str = "",
    target_track: str = "paid_local_handoff",
    context: dict[str, Any] | None = None,
    ...
    _require_role(context, {"admin"})
    target_track = target_track.strip() or "paid_local_handoff"
    if target_track not in {"local_operator_pilot", "paid_local_handoff", "hosted_saas"}:
        raise ValueError("target_track must be local_operator_pilot, paid_local_handoff, or hosted_saas")
    generated_at = _utc_timestamp()
    pack_root = RUNTIME_ROOT / "commercial_evidence_packs" / _safe_slug(f"{target_track}-{generated_at}")
    output_dir_path = Path(output_dir).expanduser().resolve() if output_dir.strip() else pack_root

    claim = claim_confirmation_summary(context=context)
    approval = commercial_approval_summary(context=context)
    release_trust = release_trust_summary(context=context)
    security_review = security_review_summary(context=context)
    hosted = hosted_readiness_summary(context=context)
    commercial = commercial_readiness()
    handoff = handoff_readiness(context=context)

    external_required = target_track in {"paid_local_handoff", "hosted_saas"}
    evidence = [
        _commercial_evidence_item(
            item_id="claim_confirmation",
            label="Claim confirmation",
            report=claim,
            accepted_statuses={"verified"},
            required=external_required,
            required_for=["paid_local_handoff", "hosted_saas"],
            path_keys=["confirmation_file", "project"],
            next_action="Configure DRAFTPAPER_CLAIM_CONFIRMATION_FILE after domain/user review confirms customer manuscript claims.",
        ),
        _commercial_evidence_item(
            item_id="commercial_approval",
            label="Commercial approval",
            report=approval,
            accepted_statuses={"verified"},
            required=external_required,
            required_for=["paid_local_handoff", "hosted_saas"],
            path_keys=["approval_file", "license_file"],
            next_action="Configure DRAFTPAPER_COMMERCIAL_APPROVAL_FILE with customer-specific legal or contract approval evidence.",
        ),
        _commercial_evidence_item(
            item_id="release_trust",
            label="Release trust",
            report=release_trust,
            accepted_statuses={"verified"},
            required=external_required,
            required_for=["paid_local_handoff", "hosted_saas"],
            path_keys=["trust_file", "release_zip", "release_manifest", "release_sha256_file", "release_signature", "release_public_key"],
            next_action="Configure DRAFTPAPER_RELEASE_TRUST_FILE and release sidecar paths for certificate, notarization, or third-party release trust evidence.",
        ),
        _commercial_evidence_item(
            ...
            required=external_required,
            required_for=["paid_local_handoff", "hosted_saas"],
            path_keys=["review_file", "security_audit_file"],
            next_action="Configure DRAFTPAPER_SECURITY_REVIEW_FILE with formal third-party security review evidence.",
        ),
        _commercial_evidence_item(
            item_id="hosted_readiness",
            label="Hosted readiness",
            report=hosted,
            accepted_statuses={"ready"},
            required=target_track == "hosted_saas",
            required_for=["hosted_saas"],
            path_keys=["hosted_readiness_file"],
            next_action="Configure DRAFTPAPER_HOSTED_READINESS_FILE with verified SSO, payment, storage/DR, worker isolation, and deployment hardening evidence.",
        ),
    ]
    blocking_items = [item for item in evidence if item.get("blocking")]
    status = "ready" if not blocking_items else "attention"
    next_actions = []
    for item in blocking_items:
        if item.get("next_action"):
            next_actions.append(str(item["next_action"]))
    next_actions.extend(str(item) for item in handoff.get("next_required_actions") or [])
    next_actions.extend(str(item) for item in commercial.get("remaining_gaps") or [])
    next_actions = list(dict.fromkeys(item for item in next_actions if item))

    output_dir_path.mkdir(parents=True, exist_ok=True)
    output_dir_path.chmod(0o700)
    json_path = output_dir_path / "commercial-evidence-pack.json"
    markdown_path = output_dir_path / "commercial-evidence-pack.md"
    pack = {
        ...
        "readiness_tracks": commercial.get("readiness_tracks") or [],
        "evidence": evidence,
        "blocking_evidence_ids": [str(item.get("id")) for item in blocking_items],
        "next_actions": next_actions,
        "reports": {
            "claim_confirmation": claim,
            "commercial_approval": approval,
            "release_trust": release_trust,
            "security_review": security_review,
            "hosted_readiness": hosted,
        },
        "notes": [
            "This evidence pack is private operator material for commercial handoff review.",
            "Draft or unconfigured evidence remains blocking for paid and hosted tracks.",
...
async function buildSupportBundleFile(){{const payload={{output_path:supportOutputPath.value.trim()||launchSupportBundlePath.value.trim()}}; const p=await api('/api/support-bundle-file',{{method:'POST',body:JSON.stringify(payload)}}); if(p.zip_path&&!supportOutputPath.value.trim())supportOutputPath.value=p.zip_path; if(p.zip_path&&!launchSupportBundlePath.value.trim())launchSupportBundlePath.value=p.zip_path; show(p);await refreshReadiness()}}
...
async function buildReleasePackage(){{const payload={{output_dir:releaseOutputDir.value.trim(),version_label:releaseVersionLabel.value.trim(),signing_key:releaseSigningKey.value.trim(),signing_public_key:releasePublicKey.value.trim(),require_signature:releaseRequireSignature.checked}}; const p=await api('/api/release-package',{{method:'POST',body:JSON.stringify(payload)}}); if(p.zip_path&&!releaseZipPath.value.trim())releaseZipPath.value=p.zip_path; if(p.zip_path&&!installReleaseZipPath.value.trim())installReleaseZipPath.value=p.zip_path; if(p.manifest_path&&!installManifestPath.value.trim())installManifestPath.value=p.manifest_path; if(p.sha256_path&&!installSha256Path.value.trim())installSha256Path.value=p.sha256_path; if(p.signature?.signature_path&&!installSignaturePath.value.trim())installSignaturePath.value=p.signature.signature_path; if(p.signature?.public_key_path&&!installPublicKeyPath.value.trim())installPublicKeyPath.value=p.signature.public_key_path; show(p);await refreshReadiness()}}
async function buildCommercialLaunchPackage(){{const payload={{acceptance_report:launchAcceptanceReportPath.value.trim(),output_dir:launchOutputDir.value.trim(),output_zip:launchOutputZipPath.value.trim(),target_track:launchTargetTrack.value,customer_id:handoffCustomerId.value.trim()||hostedCustomerId.value.trim(),customer_name:handoffCustomerName.value.trim()||hostedCustomerName.value.trim(),release_zip:releaseZipPath.value.trim()||installReleaseZipPath.value.trim(),release_manifest:launchReleaseManifestPath.value.trim()||installManifestPath.value.trim(),release_sha256_file:launchSha256Path.value.trim()||installSha256Path.value.trim(),release_signature:launchSignaturePath.value.trim()||installSignaturePath.value.trim(),release_public_key:launchPublicKeyPath.value.trim()||installPublicKeyPath.value.trim(),handoff_dossier:launchHandoffDossierPath.value.trim()||installHandoffDossierPath.value.trim(),support_bundle:launchSupportBundlePath.value.trim(),hosted_readiness_dossier:hostedDossierZipPath.value.trim()}}; const p=await api('/api/commercial-launch-package',{{method:'POST',body:JSON.stringify(payload)}}); if(p.zip_path&&!opsLaunchPackagePath.value.trim())opsLaunchPackagePath.value=p.zip_path; show(p);await refreshReadiness()}}
...
async function buildEvidencePack(){{const payload={{output_dir:evidenceOutputDir.value.trim(),target_track:evidenceTargetTrack.value}}; const p=await api('/api/commercial-evidence-pack',{{method:'POST',body:JSON.stringify(payload)}}); if(p.output_dir&&!evidenceOutputDir.value.trim())evidenceOutputDir.value=p.output_dir; show(p);await refreshReadiness()}}
async function installVerifiedRelease(){{const payload={{release_zip:installReleaseZipPath.value.trim()||releaseZipPath.value.trim(),install_root:installRootPath.value.trim(),release_manifest:installManifestPath.value.trim(),release_sha256_file:installSha256Path.value.trim(),release_signature:installSignaturePath.value.trim(),release_public_key:installPublicKeyPath.value.trim(),handoff_dossier:installHandoffDossierPath.value.trim(),require_signature:installRequireSignature.checked,require_dossier:installRequireDossier.checked,run_smoke:installRunSmoke.checked,smoke_output:installSmokeOutputPath.value.trim(),force:installForce.checked}}; const p=await api('/api/verified-release-install',{{method:'POST',body:JSON.stringify(payload)}});show(p);await refreshReadiness()}}
...
refreshBtn.onclick=refresh; refreshReadinessBtn.onclick=refreshReadiness; accessBtn.onclick=async()=>show(await api('/api/access-policy')); usageBtn.onclick=async()=>show(await api('/api/usage')); quotaBtn.onclick=async()=>show(await api('/api/quota')); billingBtn.onclick=async()=>show(await api('/api/billing')); licenseBtn.onclick=async()=>show(await api('/api/license')); entitlementsBtn.onclick=async()=>show(await api('/api/license-entitlements')); releasePackageBtn.onclick=buildReleasePackage; launchPackageBtn.onclick=buildCommercialLaunchPackage; paidConfigBtn.onclick=generatePaidConfig; suiteBtn.onclick=runCommercialSuite; opsReportBtn.onclick=buildOperationsReport; evidencePackBtn.onclick=buildEvidencePack; installBtn.onclick=installVerifiedRelease; claimsBtn.onclick=async()=>show(await api('/api/claim-confirmation')); approvalBtn.onclick=async()=>show(await api('/api/license-approval')); approvalDraftBtn.onclick=async()=>{{show(await api('/api/commercial-approval-draft',{{method:'POST',body:'{{}}'}}));await refreshReadiness()}}; trustBtn.onclick=async()=>show(await api('/api/release-trust')); trustDraftBtn.onclick=prepareTrustDraft; secReviewBtn.onclick=async()=>show(await api('/api/security-review')); secReviewDraftBtn.onclick=async()=>{{show(await api('/api/security-review-draft',{{method:'POST',body:'{{}}'}}));await refreshReadiness()}}; hostedDraftBtn.onclick=prepareHostedDraft; hostedFinalizeBtn.onclick=finalizeHostedReadiness; hostedDossierBtn.onclick=buildHostedDossier; hostedAcceptBtn.onclick=runHostedAcceptance; backupsBtn.onclick=async()=>show(await api('/api/backups')); rehearsalsBtn.onclick=async()=>show(await api('/api/backups/rehearsals')); auditBtn.onclick=async()=>show(await api('/api/audit?limit=50')); securityBtn.onclick=async()=>show(await api('/api/security-audit')); supportBtn.onclick=downloadSupportBundle; supportFileBtn.onclick=buildSupportBundleFile; readinessBtn.onclick=async()=>show(await api('/api/commercial-readiness')); handoffBtn.onclick=async()=>show(await api('/api/handoff-readiness')); cleanupJobsBtn.onclick=async()=>{{show(await api('/api/jobs/cleanup',{{method:'POST',body:JSON.stringify({{keep:20}})}}));await refreshJobs()}}; createBtn.onclick=async()=>{{const p=await api('/api/jobs',{{method:'POST',body:JSON.stringify({{action:'create-project',idea:idea.value,field:field.value,target_journal:targetJournal.value||'General Academic Journal'}})}});show(p);await refreshJobs();await refreshReadiness()}}; runBtn.onclick=async()=>start(sel.value); statusBtn.onclick=async()=>start('status'); syncBtn.onclick=async()=>start('sync-artifact-stale'); setInterval(refreshJobs,2500); refresh().catch(show);
</script></body></html>"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            elif parsed.path == "/api/support-bundle-file":
                _json(
                    self,
                    build_support_bundle_file_from_console(
                        output_path=str(payload.get("output_path") or ""),
                        context=context,
                    ),
                    HTTPStatus.CREATED,
                )
            elif parsed.path == "/api/commercial-evidence-pack":
                _json(
                    self,
                    build_commercial_evidence_pack_from_console(
            ...
            elif parsed.path == "/api/release-package":
                _json(
                    self,
                    build_release_package_from_console(
                        output_dir=str(payload.get("output_dir") or ""),
                        ...
                        require_signature=bool(payload.get("require_signature")),
                        context=context,
                    ),
                    HTTPStatus.CREATED,
                )
            elif parsed.path == "/api/commercial-launch-package":
                _json(
                    self,
                    build_commercial_launch_package_from_console(
                        acceptance_report=str(payload.get("acceptance_report") or ""),
                        output_dir=str(payload.get("output_dir") or ""),
                        output_zip=str(payload.get("output_zip") or ""),
                        target_track=str(payload.get("target_track") or "paid_local_handoff"),
                        customer_id=str(payload.get("customer_id") or ""),
                        customer_name=str(payload.get("customer_name") or ""),
                        release_zip=str(payload.get("release_zip") or ""),
                        release_manifest=str(payload.get("release_manifest") or ""),
                        release_sha256_file=str(payload.get("release_sha256_file") or ""),
                        release_signature=str(payload.get("release_signature") or ""),
                        release_public_key=str(payload.get("release_public_key") or ""),
                        handoff_dossier=str(payload.get("handoff_dossier") or ""),
                        support_bundle=str(payload.get("support_bundle") or ""),
            ...
            elif parsed.path == "/api/verified-release-install":
                _json(
                    self,
                    install_verified_release_from_console(
                        release_zip=str(payload.get("release_zip") or ""),
                        ...
                        handoff_dossier=str(payload.get("handoff_dossier") or ""),
                        require_signature=bool(payload.get("require_signature")),
                        require_dossier=bool(payload.get("require_dossier")),
                        run_smoke=bool(payload.get("run_smoke", True)),
                        smoke_output=str(payload.get("smoke_output") or ""),
